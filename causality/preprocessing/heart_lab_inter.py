...
class Core(object):
    ...
    def __init__(self, inPath,core = '35'):
        ...
        diagno_df_all = self.spark.sql("select SUBJECT_ID, HADM_ID, SEQ_NUM, floor(cast(ICD9_CODE as int)/10) as ICD9_CODE from dia")
        diagno_df_all = diagno_df_all.filter(diagno_df_all.ICD9_CODE.isNotNull())
        diagno_df_all.show()
        diagno_df_1 = diagno_df_all.filter(diagno_df_all.ICD9_CODE < 430)
        diagno_df_1 = diagno_df_1.filter(diagno_df_1.ICD9_CODE >= 390)
        diagno_df_1.registerTempTable("diagonise1")
        patient_list_1 = self.spark.sql("select distinct SUBJECT_ID from diagonise1")

        diagno_df_2 = diagno_df_all.filter(diagno_df_all.ICD9_CODE < 590)
        diagno_df_2 = diagno_df_2.filter(diagno_df_2.ICD9_CODE >= 580)
        diagno_df_2.registerTempTable("diagonise2")
        patient_list_2 = self.spark.sql("select distinct SUBJECT_ID from diagonise2")

        # patient_list = patient_list_1.select("SUBJECT_ID").subtract(patient_list_2.select("SUBJECT_ID"))
        patient_list = patient_list_1.select("SUBJECT_ID").intersect(patient_list_2.select("SUBJECT_ID"))
        ...
        self.patient.show()
        marks.write.csv("file:///home/zcjin/Data_event_anomaly_inter")
        result.write.csv("file:///home/zcjin/Data_connect_anomaly_inter")
        self.patient.write.csv("file:///home/zcjin/Data_patient_anomaly_inter")
