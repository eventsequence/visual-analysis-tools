...
def Initialization_Basis(Seqs=None):
    ...
    model["delete"] = np.ones([D,len(model["landmark"]),D])
    model["add"] = np.ones([D, len(model["landmark"]), D])

    return model

def Learning_granger(Seqs=None, model=None, alg=None):
    # initial
    Aest = model["A"]
    delete = model['delete']
    add = model['add']
    ...
    for o in range(0, alg["outer"]):
        ...
        for n in range(0, alg["inner"]):
            ...
            if alg["Sparse"] == 0 and alg["GroupSparse"] == 0 and alg["LowRank"] == 0:
                ...
                AmatA = AmatA * add
                A = (- BmatA + np.sqrt(BmatA ** 2 - np.dot(4, AmatA) * CmatA)*add) / (np.dot(2, AmatA))
                A[np.isnan(A)] = 0
                A[np.isinf(A)] = 0
            # check convergence
...
if __name__ == '__main__':
    ...
    # model['delete'][5,:,0] = 0
    model['add'][5,:4] = -1
    model1 = Learning_granger(Seq, model,alg1)
