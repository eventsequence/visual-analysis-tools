...
def padMatrixWithTime(seqs, times, options):
	...
	for idx, (seq,time) in enumerate(zip(seqs,times)):
		...
		for xvec, subseq in zip(x[:,idx,:], seq):
		  xvec[subseq] = 1.0
		#print time
		#print(len(lengths[idx], idx))
		t[:lengths[idx], idx] = time

	if options['useLogTime']: t = np.log(t + 1.0)
...
def load_data(dataFile, timeFile):
	...
	test_set = (test_set_x, test_set_t)
	...
	for index in range(len(testSet[0])):
		...
		for i in range(len(patient)):
		  	...
		  	for j in range(len(visit)):
		  		code = visit[j]
		  	#print params['w_output'].T.shape
		  	#print code
		  	#print params['W_emb'].shape
		  		contribution = np.dot(params['w_output'].T, alpha[i] * beta[i] * params['W_emb'][code])
		  		contribution_list.append(contribution.tolist())
        	contribution_time.append(contribution_list)
	  	y_pre.append(y_t.tolist())
	# print y_pre
	return y_pre, contribution_time
