...
def alignment(cur_patient, sim_set):
    sankey_out = {}

    data_all = {}
    seqvec_all = []

    cur_pid = cur_patient['pid']
    cur_seq = cur_patient['sequence']
    cur_vec = []

    cut_index_set = {}

    with open('data/cut_index.json','r') as f:
        cut_index_set = json.load(f)
    ...
    for pid, data in data_all.items():
        ...
        data_all[pid]['cut_index'] = cut_index_set[cur_pid][pid]
    ...
    for l in range(len(min_path_group)):
        ...
        for ll in range(new_current_grouped[l][0],(new_current_grouped[l][-1]+1)):
            tmp1.extend(cur_seq[cur_visit_group[ll][0]:(cur_visit_group[ll][-1]+1)])
