...
export const seqquery = function() {
    ...
    seqquery.disease = function(_) {
        if(!arguments.length) return disease;
        disease = _;
        return seqquery;
    }

    seqquery.treatment = function(_) {
    ...
    seqquery.addEvent = function(item) {
        ...
        let currentContainHeight = container.attr('height');
        if(containHeight > currentContainHeight) container.attr('height', containHeight)
        if (count===0) {
        ...
        itembox.append('text')
            ...
            .text((d)=> {
                ...
                // console.log(disease)
                if(d['event_type'] === 'Treatments') {
                    let label = treatment[d['icd_code']];
                    if(label.length > 16) {
        ...
        btns.select('.delete-btn')
            .on('click', function(){
                ...
                curItembox.remove()
                seqquery.deleteItem(Number(itemboxIdx))
            })
        count++
        ...
        $(".event-text").tipsy({
            ...
            title: function() {
                ...
                let tip = '';
                if(d['event_type'] === 'Treatments') {
                    tip = treatment[d['icd_code']];
                } else {
                    tip = disease[d['icd_code']];
                }
                return tip;
            }
        });
    ...
    seqquery.deleteItem = function(deleteIdx) {
        container.selectAll('.itembox')
        .filter(function(){
            let itemboxId = d3.select(this).attr('id');
            let idx = itemboxId.split('_')[1];
            if (Number(idx) > deleteIdx) {
                return true;
            } else {
                return false;
            }
        ...
        .attr('transform', function(){
            let itemboxId = d3.select(this).attr('id');
            let idx = itemboxId.split('_')[1];
            return `translate(${strokeWidth/2},${strokeWidth/2+ idx*(itemboxHeight+itemboxPadding)})`
        })
        count--;
        let containHeight = itemboxHeight * count + itemboxPadding * (count-1)
        let currentHeight = container.attr('height')
        if(containHeight < currentHeight && containHeight >0) container.attr('height', containHeight)
        return seqquery;
    }
