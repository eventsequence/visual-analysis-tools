...
export const Simvis = function() {
    ...
    simvis.loading = function() {
        if (container !== null) {
            d3.select('#similiar-slider').style('display','none')
    ...
    simvis.layout = function() {
        ...
        total_height = (old_seq_data.length)*threadHeight + 15

        total_height = Math.max(cluster.cluster_height+seq_height+15,total_height)
        // sankey = d3.sankey()
    ...
    simvis.render = function(){
        ...
        $('#similar-container').scroll(function () {
            ...
            d3.select('.origin-container').attr('transform', () =>{
                let newTransX = 50 - sl;
                return `translate(${newTransX}, 80)`
            })
        });
    ...
    simvis.drawcluster =function(){
        ...
        $('.sankey_node').tipsy({
          ...
          title:function(){
            ...
            if(cur_type=='Treatments'){
                ...
                tip += "<div class='tooltip-item'>Name: "+treatment[cur_icd_code]+"</div>"
                tip += "<div class='tooltip-item'>Patient Count: "+number+"</div>"
                tip += "<div class='tooltip-item'>Patient Percentage: "+Math.round(patientPercent*100)+"%</div>"
                tip += "</div>"

            } else {
                tip += "<div class='rect-tooltip'>"
                tip += "<div class='tooltip-item'>Diagnosis</div>"
                tip += "<div class='tooltip-item'>ICD-9: "+cur_icd_code+"</div>"
                tip += "<div class='tooltip-item'>Name: "+disease[cur_icd_code]+"</div>"
                tip += "<div class='tooltip-item'>Patient Count: "+number+"</div>"
                tip += "<div class='tooltip-item'>Patient Percentage: "+Math.round(patientPercent*100)+"%</div>"
                tip += "</div>"
            }
        ...
        $("#sim-canvas").on('click',function(){
            ...
            if(show_flag == 'sequence'&& !onRect) {
                let seqNodes = container.selectAll('.seq_node.filtered')
                ...
                let resultNodes = d3.selectAll(".result_node.filtered")
                .classed('filtered', false)
                resultNodes.select('rect')
                .style("fill-opacity",1).style("stroke-opacity",1)
                resultNodes.select('text')
                .style("fill-opacity",function(d){
                    var cur_list = d3.select(this.parentNode.parentNode).data()[0]
                    cur_list = _.uniq(cur_list,function(item,key,a){
                        return item.icd_code
        ...
        $("#currentseq-canvas").on('click',function(){
            if(show_flag=='cluster'&&!onRect){
                d3.selectAll(".sankey_node")
                    .classed("highlight",true)
        ...
        // $( ".sankey_node" ).on('contextmenu', function() {
        //     console.log('dblclick')
        // })
        $("#similar-container").bind("contextmenu", function(){
            return false;
        })
        $(".sankey_node").mousedown(function(e) {
            if (e.which==3) {
                if (d3.select(this).classed('highlight')) {
                    var nodes = [], links = [];
                    let stageId = d3.select(this).attr('id').split('_')[1];
                    let currentName = d3.select(this).attr('name');
                    console.log(stageId);
                    container.selectAll('#sankey_'+stageId)
                    .filter(function () {
                        let compareName = d3.select(this).attr('name');
                        if (currentName !== compareName && d3.select(this).classed('highlight')) {
                            return true;
                        } else {
                            return false;
                        }
                    ...
                    .each(function(){
                        ...
                        let nodeData  = d3.select(this).data()[0];
                        let getForward = find_forwards(nodeData, {'node':[nodeData.name],'link':[]} )
                        let getBackward = find_backwards(nodeData, {'node':[],'link':[]} )
                        nodes = nodes.concat(getForward['node'])
                        nodes = nodes.concat(getBackward['node'])
                        links = links.concat(getForward['link'])
                        links = links.concat(getBackward['link'])
                    })
                    var getResult = {'node': nodes, 'link': links}
                    var result_node = {}
                    var result_link = {}
        ...
        $( ".sankey_node" ).on('mouseenter', function() {
            ...
            // d3.selectAll(".sankey_text text").style("fill-opacity",0.1)
               ...
               let current_eventcode = d3.select(this).select('rect').attr('id').split('_')[1];
                let current_stage = d3.select(this).attr('id').split('_')[1]
        ...
        $( ".sankey_node" ).on('mouseleave', function(d) {
            var current_stage = d3.select(this).attr('id').split('_')[1];
    ...
    simvis.drawPredict = function(){
        ...
        container.select('.load').remove();
        d3.select('#similiar-slider').style('display','block')
        ...
        originCanvas = d3.select('#similiar-slider')
        .select('svg')
        .append("g")
        .attr("class","origin-container")
        .attr("transform","translate(50,80)")

        originCanvas.append('line')
        .attr("id","currentseq_divide_line")
        .attr('x1', his_width+connect/2)
        .attr('x2', his_width+connect/2)
        .attr('y1',-20)
        .attr('y2', 50)
        .attr('stroke', '#BFBFBF')
        .attr('stroke-width',2)
        ...
        for(var i=0; i<selected_disease.length; i++){
            ...
            predict_draw.push({
                ...
                'time': predict_result[selected_disease[i]]['time']
            })
        }
        ...
        result_origin.append("g")
            ...
            .attr('fill',function(d){
              if(d['data']['name'].split('-').length){
                let color = diseaseColors[d['data']['name'].split('-')[2]]
                let colorscale = d3.scaleLinear().domain([0,1]).range(['#FFFFFF', color]);
                return colorscale(Math.pow(d['data'].confidence, 1));
              }
            })
        ...
        $('.result_origin').tipsy({
          ...
          title:function(){
            ...
            tip += "<div class='tooltip-item'>Prevalence: "+Math.round(confidence*1000)/10+"%</div>"
            tip += "<div class='tooltip-item'>Time: "+time.split(' ')[0]+"</div>"
            tip += "</div>"
        ...
        $(".result_origin").on('mouseenter', function() {
            ...
            if(show_flag=="cluster"){
                ...
                // console.log(icdcode)
                let endIcdcodes = icdcode.split('|');
                let sankeyNode = container
                .selectAll('#sankey_'+stage_num)
                .filter(function(d){
                    let compare_eventcode = d3.select(this).attr('label');
                    let compare_icdcode = type_reves[compare_eventcode];
                    if(compare_icdcode >= Number(endIcdcodes[0]) && compare_icdcode <= Number(endIcdcodes[1])) {
                        return true;
                    } else{
                        return false;
                    }
                ...
                if(!sankeyNode.empty()) {
                    var nodes = [], links = [];
                    // console.log(sankeyNode.data())
                    for (let i=0; i < sankeyNode.data().length; i++) {
                        let nodeData = sankeyNode.data()[i];
        ...
        $(".result_origin").on('mouseleave', function(d) {
            if(show_flag=='sequence'){
                ...
                let resultNodes = d3.selectAll("#resultstage_"+current_stage+" .result_node.filtered")
                
                resultNodes.select('rect')
                .style("fill-opacity",0.1).style("stroke-opacity",0.1)
                resultNodes.select('text')
                .style("fill-opacity",function(d){
                    var cur_list = d3.select(this.parentNode.parentNode).data()[0]
                    cur_list = _.uniq(cur_list,function(item,key,a){
        ...
        $( ".result_origin" ).on('click', function() {
            ...
            if(show_flag=='sequence'){
                ...
                var current_code = type[current_icd]
                var current_stage = 0
                let resultNodes = d3.selectAll("#resultstage_"+current_stage+" .result_node")
                ...
                for(var index=start_icd; index<=end_icd; index++){
                    if(type[index]){
                        var current_code = type[index]
                        let hightlightNodes = d3.selectAll("#resultstage_"+current_stage+" #tree_"+current_code)
                        .classed('filtered', false)
                        hightlightNodes
                        .select('rect')
                        .style("fill-opacity",1).style("stroke-opacity",1)
                        hightlightNodes.select('text').style("fill-opacity",function(d){
                            var cur_list = d3.select(this.parentNode.parentNode).data()[0]
                        cur_list = _.uniq(cur_list,function(item,key,a){
                          return item.icd_code
                        })
        ...
        let origin_node = origin_stage.selectAll(".origin_node")
            .data(function(d){
                ...
                for (let i = 0; i < cur_list.length; i++) {
                    let item = cur_list[i];
                    let name = item.event_type + '-' + item.event_code + '-' +item.stage;
                    cur_time_obj[name].push(item.event_time);
                }
      
                var count=[];
                for(var key in cur_obj){
                  count.push([key,cur_obj[key],cur_time_obj[key]]);
                }
        ...
        progress_origin.append("rect")
            ...
            .attr("width",function(d,i){
                if(stage_min_max[1]==stage_min_max[0]||d['time']===0){
                    ...
                    let width = ((Math.log(d['time']))/(Math.log(stage_min_max[1])))*(connect-3)
...
/*----------------------------------------------------------*/
        ...
        progress_stage.append("rect")
            ...
            .attr("width",function(d,i){
                if(stage_min_max[1]==stage_min_max[0]|| d['time'] == 0){
                    ...
                    return ((Math.log(d['time']))/(Math.log(stage_min_max[1])))*(connect-3)
        ...
        let seq_node = seq_stage.selectAll(".seq_node")
            .data(function(d){
                ...
                for (let i = 0; i < cur_list.length; i++) {
                    let item = cur_list[i];
                    let name = item.event_type + '-' + item.event_code + '-' +item.cluster+'-'+item.stage;
                    cur_time_obj[name].push(item.event_time);
                }
...
/*----------------------------------------------------------*/
    ...
    function addtipsy() {
        ...
        $('.origin_node').tipsy({
            ...
            title:function(){
                ...
                cur_event.event_type = cur_event.name.split('-')[0]
                let timeList = cur_event.time;
                var tip = ''
                if(cur_event.event_type=='Treatments'){
                    tip += "<div class='rect-tooltip'>"
                    tip += "<div class='tooltip-item'>Treatment</div>"
                    tip += "<div class='tooltip-item'>Name: "+treatment[cur_event.icd_code]+"</div>"
                ...
                tip += "<div class='tooltip-item'>Frequency: "+cur_event.size+"</div>"
                if (timeList.length <=1) {
                    tip += "<div class='tooltip-item'>Time: "+timeList[0]+"</div>"
                } else {
                    tip += "<div class='tooltip-item'>First Time: "+timeList[0]+"</div>"
                    tip += "<div class='tooltip-item'>Last Time: "+timeList[timeList.length-1]+"</div>"
    
                    // for (let i = 0; i < timeList.length; i++) {
                    //     tip += "<div class='tooltip-item'>"+timeList[i]+"</div>"
                    // }
                }
                tip += "</div>"
                return tip
        ...
        $( ".origin_node" ).on('mouseenter', function() {
            ...
            if(show_flag=='sequence'){
                ...
                let seqNodes = d3.selectAll("#stage_"+current_stage+" .seq_node")
                

                seqNodes.select('rect')
                .style("fill-opacity",0.1)
                .style("stroke-opacity",0.1)
                seqNodes.select('text')
                .style("fill-opacity",function(d){
                    var cur_list = d3.select(this.parentNode.parentNode).data()[0]
                    cur_list = _.uniq(cur_list,function(item,key,a){
                    return item.icd_code
                ...
                let hightlightNodes = d3.selectAll("#stage_"+current_stage+" #tree_"+current_code)
            

                hightlightNodes.select('rect')
                .style("fill-opacity",1)
                .style("stroke-opacity",1)

                hightlightNodes.select('text')
                .style("fill-opacity",function(d){
                    var cur_list = d3.select(this.parentNode.parentNode).data()[0]
                    cur_list = _.uniq(cur_list,function(item,key,a){
                    return item.icd_code
            ...
            if(show_flag=="cluster"){
                let current_eventcode =d3.select(this).select('rect').attr('name').split('-')[1];
                let stage_idx = d3.select(this.parentNode).attr('id').split('_')[1];
                let sankeyNode = container
                .selectAll('#sankey_'+stage_idx)
                .filter(function(d){
                    let compare_eventcode = d3.select(this).attr('label');
                    if(compare_eventcode === current_eventcode) {
                        return true;
        ...
        $( ".origin_node" ).on('mouseleave', function(d) {
            ...
            if(show_flag=='sequence'){
                ...
                d3.selectAll("#stage_"+current_stage+" .seq_node.filtered rect").style("fill-opacity",0.1).style("stroke-opacity",0.1)
                d3.selectAll("#stage_"+current_stage+" .seq_node.filtered text").style("fill-opacity",function(d){
                    var cur_list = d3.select(this.parentNode.parentNode).data()[0]
                    cur_list = _.uniq(cur_list,function(item,key,a){
                        return item.icd_code
                    })
    ...
    function dragstart() {
        ...
        .on("mousemove", ()=>{
            ...
            d3.select('.origin-container').attr('transform', () =>{
                let newTransX = 50 - sliderScale.invert(newX);
                return `translate(${newTransX}, 80)`
            })
        })
        .on("mouseup", mouseup);
