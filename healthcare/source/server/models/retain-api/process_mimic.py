...
def pre_mimic(Disease):
	admissionFile = './origin/ADMISSIONS.csv'
	diagnosisFile = './origin/DIAGNOSES_ICD.csv'
	patientsFile = './origin/PATIENTS.csv'
	outFile = './data/mimic'

	print 'Collecting mortality information'
	# Disease=['D_401','D_402','D_403','D_404']
	pidDodMap = {}
	...
	for line in infd:
		...
		admTime = time.strptime(tokens[3], '%Y-%m-%d %H:%M:%S')
	...
	for pid, visits in pidSeqMap_3digit.iteritems():
		...
		for i in range(len(Disease)):
			# print seq[-1]
			if Disease[i] in seq[-1]:
				print "!!!!!!!!!!!!!!!!!!!!!!!!!!!"
				pidDodMap[pid].append(1)
			else:
				pidDodMap[pid].append(0)
		print pidDodMap[pid]
