...
def build_model(tparams, options, W_emb=None):
	...
	# attention = theano.scan(fn=getAttention, sequences=T.arange(n_input), outputs_info=None, name='getAttention', n_steps= n_input)
	...
	print 'shape'
	preY = preY.reshape((preY.shape[0], preY.shape[1], preY.shape[2]))
	indexRow = T.arange(n_samples)
	y_hat = preY[lengths - 1, indexRow, :]
	y_hat = y_hat.reshape((preY.shape[1],preY.shape[2]))
...
def calculate_auc(test_model, dataset, options):
	...
	print "!!!!!!!!!!!!!!!!"
	labels = dataset[1]
	auc = roc_auc_score(np.array(labels), np.array(scoreVec), average= 'macro')
	return auc, scoreVec[0][0]
...
def train_RETAIN(
	...
	if useTime and embFineTune:
		...
		# get_pre = theano.function(inputs=[x, t], outputs=preY, name='get_prediction')
	...
	for epoch in xrange(max_epochs):
		...
		validAuc, pre = calculate_auc(get_prediction, validSet, options)
		...
		if validAuc > bestValidAuc:
			...
			for i in range(len(testSet2[-1])):
				for j in range(len(testSet2[-1][i])):
					testSet2[-1][i][j] -= testSet[-1][i][-1]/2
			print testSet2[-1][0][0]
			print testSet[-1][0][0]
			bestTestAuc, pre = calculate_auc(get_prediction, testSet, options)
			bestTestAuc2, pre2 = calculate_auc(get_prediction, testSet2, options)
			buf = 'Currently the best validation AUC found. Test AUC:%f, %fat epoch:%d' % (bestTestAuc,pre, epoch)
			buf2 = 'Currently the best validation AUC found. Test AUC:%f, %f at epoch:%d' % (bestTestAuc2,pre2, epoch)
			print buf
			print buf2
			print2file(buf, logFile)
